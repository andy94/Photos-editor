...
/**
 * Componenta care salveaza imaginea
 *
 */
public class ImageSaver extends Component {
	...
	public Message notify(Message message) {
		...
		BufferedImage bufferedImage = new BufferedImage(save.getWidth(),
				save.getHeight(), BufferedImage.TYPE_3BYTE_BGR);
		int[][][] pixels = save.getPixels();
		for (int i = 0; i < bufferedImage.getHeight(); i++) {
			for (int j = 0; j < bufferedImage.getWidth(); j++) {
				bufferedImage.setRGB(j, i, (pixels[i][j][0] << 16)
						+ (pixels[i][j][1] << 8) + (pixels[i][j][2] << 0));
			}
		}
