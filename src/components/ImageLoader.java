...
/**
 * Componenta care citeste imaginea
 *
 */
	...
	public Message notify(Message message) {
		...
		try {
			BufferedImage bufferedImage = ImageIO
					.read(new File(load.getPath()));
			MessageImage messageImage = new MessageImage(null);

			byte[] pixelsByte = ((DataBufferByte) bufferedImage.getRaster()
					.getDataBuffer()).getData();
			int index = 0;
			int[][][] pixels = new int[bufferedImage.getHeight()][bufferedImage
					.getWidth()][3];
